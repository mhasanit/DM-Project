...
class Node2Vec(object):
    ...
    def read_graph(self, file):
        ...
        if self.is_weighted:
            ...
            self.G = nx.read_edgelist(file, data=(('weight', float),), create_using=nx.DiGraph(), edgetype=str)

        else:

            self.G = nx.read_edgelist(file, create_using=nx.DiGraph(), edgetype=str)
    ...
    def learn_embeddings(self, output, output_format='text'):
        """
        ...
        model = Word2Vec(self._walks, vector_size=self.dimensions, window=self.window_size, min_count=0,
                         workers=self.workers,epochs=1, negative=25, sg=1)
    ...
    def run(self, input_graph, output, output_format='text'):
...
if __name__ == '__main__':
    ...
    emb_df = pd.read_csv('output.txt', sep=' ', skiprows=[0], header=None)
